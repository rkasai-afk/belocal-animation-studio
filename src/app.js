...
function computeCameraTarget(scope, regionName, padding, manual) {
  if (manual && typeof manual.zoom === 'number') {
    return { scaleMultiplier: Math.max(1, Math.min(MAX_CAMERA_ZOOM, manual.zoom)), dxNative: manual.x || 0, dyNative: manual.y || 0 };
  }
...
function buildPlaneShape(color) {
  return new fabric.Path('M 0 -11 L 3.5 -2 L 11 3 L 3 3.6 L 3.6 10 L 0 7 L -3.6 10 L -3 3.6 L -11 3 L -3.5 -2 Z', {
    fill: color, stroke: 'rgba(0,0,0,0.25)', strokeWidth: 0.6, originX: 'center', originY: 'center', selectable: false, evented: false,
  });
}
function buildArrowHeadShape(color) {
  return new fabric.Triangle({ width: 18, height: 20, fill: color, originX: 'center', originY: 'center', selectable: false, evented: false });
}
...
function buildMapGroup(scope, events) {
  ...
  events.filter(e => e.type === 'route' && e.from && e.to).forEach(e => {
    ...
    const lineShape = new fabric.Path(d, {
      ...
      strokeDashArray: e.dashed ? [11, 9] : null, opacity: e.showTrail === false ? 0 : 0.85, selectable: false, evented: false,
    });
    const originDot = e.originMarker !== 'none' ? new fabric.Circle({ left: geo.from.x, top: geo.from.y, radius: 6, fill: color, stroke: '#FFFFFF', strokeWidth: 1.5, originX: 'center', originY: 'center', selectable: false, evented: false }) : null;
    const destDot = e.destMarker !== 'none' ? new fabric.Circle({ left: geo.to.x, top: geo.to.y, radius: 6, fill: color, stroke: '#FFFFFF', strokeWidth: 1.5, originX: 'center', originY: 'center', selectable: false, evented: false }) : null;
    let marker = null;
...
function applyMapTimeline(obj, elapsed) {
  ...
  if (zoomEvents.length && cfg.baseTransform) {
    ...
    for (const e of zoomEvents) {
      const target = computeCameraTarget(scope, e.region, e.padding, e.manualZoom != null ? { zoom: e.manualZoom, x: e.manualX, y: e.manualY } : null);
...
function mapEventSummary(e) {
  ...
  if (e.type === 'zoom') return `Zoom to ${e.region || '(pick a region)'}`;
  if (e.type === 'route') return `Route ${e.from || '?'} → ${e.to || '?'}`;
  if (e.type === 'stat') return `Stat: ${e.label || '(untitled)'}${e.value ? ' = ' + e.value : ''}`;
  return e.type;
}
function renderMapEventTiming(container, e, obj, onAfterChange) {
  const row = document.createElement('div'); row.className = 'row2'; container.appendChild(row);
  const c1 = document.createElement('div'); row.appendChild(c1);
  const l1 = document.createElement('label'); l1.textContent = 'Start (sec)'; l1.style.fontSize = '11px'; c1.appendChild(l1);
  const i1 = document.createElement('input'); i1.type = 'number'; i1.step = '0.1'; i1.min = 0; i1.value = (e.start / 1000).toFixed(1);
  i1.addEventListener('change', () => { e.start = Math.max(0, Math.round(parseFloat(i1.value) * 1000) || 0); onAfterChange(); });
  c1.appendChild(i1);
  const c2 = document.createElement('div'); row.appendChild(c2);
  const l2 = document.createElement('label'); l2.textContent = 'Duration (sec)'; l2.style.fontSize = '11px'; c2.appendChild(l2);
  const i2 = document.createElement('input'); i2.type = 'number'; i2.step = '0.1'; i2.min = 0.1; i2.value = (e.duration / 1000).toFixed(1);
  i2.addEventListener('change', () => { e.duration = Math.max(100, Math.round(parseFloat(i2.value) * 1000) || e.duration); onAfterChange(); });
  c2.appendChild(i2);
}
function renderMapEventList(body, obj) {
  ...
  function paint() {
    ...
    cfg.events.forEach((e, idx) => {
      ...
      const regionNames = mapRegionsFor(cfg.scope).map(r => r.name).sort();
      const makeRegionSelect = (value, onChange, allowNone) => {
        const sel = document.createElement('select');
        if (allowNone) { const o = document.createElement('option'); o.value = ''; o.textContent = '— choose —'; sel.appendChild(o); }
        regionNames.forEach(n => { const o = document.createElement('option'); o.value = n; o.textContent = n; sel.appendChild(o); });
        sel.value = value || '';
        sel.addEventListener('change', () => { onChange(sel.value); commit(); });
        return sel;
      };
      const makeSwatchRow = (value, onChange) => {
        const row = document.createElement('div'); row.className = 'swatches';
        SWATCH_OPTIONS.forEach(o => {
          const s = document.createElement('div'); s.className = 'swatch' + (value === o.val ? ' active' : '');
          s.style.background = o.val; s.title = o.name;
          s.addEventListener('click', () => { onChange(o.val); commit(); });
          row.appendChild(s);
        });
        return row;
      };
      ...
      if (e.type === 'highlight') {
        const l1 = document.createElement('label'); l1.textContent = 'Region'; l1.style.fontSize = '11px'; body2.appendChild(l1);
        body2.appendChild(makeRegionSelect(e.region, (v) => { e.region = v; }, true));
        const l2 = document.createElement('label'); l2.textContent = 'Highlight color'; l2.style.fontSize = '11px'; l2.style.marginTop = '6px'; body2.appendChild(l2);
        body2.appendChild(makeSwatchRow(e.color, (v) => { e.color = v; }));
        const dimLbl = document.createElement('label'); dimLbl.style.fontSize = '11.5px'; dimLbl.style.marginTop = '6px'; dimLbl.style.display = 'flex'; dimLbl.style.alignItems = 'center'; dimLbl.style.gap = '6px';
        const dimCb = document.createElement('input'); dimCb.type = 'checkbox'; dimCb.checked = e.dim !== false; dimCb.style.width = 'auto';
        dimCb.addEventListener('change', () => { e.dim = dimCb.checked; commit(); });
        dimLbl.appendChild(dimCb); dimLbl.appendChild(document.createTextNode('Dim other regions while this is highlighted'));
        body2.appendChild(dimLbl);
      } else if (e.type === 'zoom') {
        const l1 = document.createElement('label'); l1.textContent = 'Zoom to region'; l1.style.fontSize = '11px'; body2.appendChild(l1);
        body2.appendChild(makeRegionSelect(e.region, (v) => { e.region = v; }, true));
        const l2 = document.createElement('label'); l2.textContent = 'Padding around region'; l2.style.fontSize = '11px'; l2.style.marginTop = '6px'; body2.appendChild(l2);
        const padSel = document.createElement('select');
        [['0.35', 'Loose'], ['0.22', 'Comfortable'], ['0.1', 'Tight']].forEach(([v, label]) => { const o = document.createElement('option'); o.value = v; o.textContent = label; padSel.appendChild(o); });
        padSel.value = String(e.padding != null ? e.padding : 0.22);
        padSel.addEventListener('change', () => { e.padding = parseFloat(padSel.value); commit(); });
        body2.appendChild(padSel);
      } else if (e.type === 'route') {
        const row1 = document.createElement('div'); row1.className = 'row2'; body2.appendChild(row1);
        const fw = document.createElement('div'); const fl = document.createElement('label'); fl.textContent = 'From'; fl.style.fontSize = '11px'; fw.appendChild(fl); fw.appendChild(makeRegionSelect(e.from, (v) => { e.from = v; }, true)); row1.appendChild(fw);
        const tw = document.createElement('div'); const tl = document.createElement('label'); tl.textContent = 'To'; tl.style.fontSize = '11px'; tw.appendChild(tl); tw.appendChild(makeRegionSelect(e.to, (v) => { e.to = v; }, true)); row1.appendChild(tw);

        const l2 = document.createElement('label'); l2.textContent = 'Style'; l2.style.fontSize = '11px'; l2.style.marginTop = '6px'; body2.appendChild(l2);
        const styleRow = document.createElement('div'); styleRow.className = 'row2'; body2.appendChild(styleRow);
        [['line', 'Line'], ['arrow', 'Arrow'], ['flight', 'Flight']].forEach(([v, label]) => {
          const b = document.createElement('button'); b.type = 'button'; b.style.flex = '1';
          b.className = 'small' + (e.style === v ? ' primary' : ' ghost'); b.textContent = label;
          b.addEventListener('click', () => {
            e.style = v;
            e.movingObject = v === 'flight' ? 'plane' : v === 'arrow' ? 'arrow' : 'none';
            e.curve = v === 'flight' ? 'medium' : e.curve;
            commit();
          });
          styleRow.appendChild(b);
        });

        const l3 = document.createElement('label'); l3.textContent = 'Curve'; l3.style.fontSize = '11px'; l3.style.marginTop = '6px'; body2.appendChild(l3);
        const curveSel = document.createElement('select');
        ['straight', 'low', 'medium', 'high'].forEach(v => { const o = document.createElement('option'); o.value = v; o.textContent = v[0].toUpperCase() + v.slice(1); curveSel.appendChild(o); });
        curveSel.value = e.curve || 'medium';
        curveSel.addEventListener('change', () => { e.curve = curveSel.value; commit(); });
        body2.appendChild(curveSel);

        const l4 = document.createElement('label'); l4.textContent = 'Route color'; l4.style.fontSize = '11px'; l4.style.marginTop = '6px'; body2.appendChild(l4);
        body2.appendChild(makeSwatchRow(e.color, (v) => { e.color = v; }));
      } else if (e.type === 'stat') {
        const l1 = document.createElement('label'); l1.textContent = 'Label'; l1.style.fontSize = '11px'; body2.appendChild(l1);
        const inpLabel = document.createElement('input'); inpLabel.type = 'text'; inpLabel.value = e.label || ''; inpLabel.placeholder = 'Population';
        inpLabel.addEventListener('change', () => { e.label = inpLabel.value; commit(); });
        body2.appendChild(inpLabel);
        const row2 = document.createElement('div'); row2.className = 'row2'; row2.style.marginTop = '6px'; body2.appendChild(row2);
        const vw = document.createElement('div'); const vl = document.createElement('label'); vl.textContent = 'Value'; vl.style.fontSize = '11px'; vw.appendChild(vl);
        const inpVal = document.createElement('input'); inpVal.type = 'text'; inpVal.value = e.value || ''; inpVal.placeholder = '5.0M'; inpVal.addEventListener('change', () => { e.value = inpVal.value; commit(); }); vw.appendChild(inpVal); row2.appendChild(vw);
        const uw = document.createElement('div'); const ul = document.createElement('label'); ul.textContent = 'Unit (optional)'; ul.style.fontSize = '11px'; uw.appendChild(ul);
        const inpUnit = document.createElement('input'); inpUnit.type = 'text'; inpUnit.value = e.unit || ''; inpUnit.placeholder = 'people'; inpUnit.addEventListener('change', () => { e.unit = inpUnit.value; commit(); }); uw.appendChild(inpUnit); row2.appendChild(uw);
        const l3 = document.createElement('label'); l3.textContent = 'Secondary line (optional)'; l3.style.fontSize = '11px'; l3.style.marginTop = '6px'; body2.appendChild(l3);
        ...
        const cuCb = document.createElement('input'); cuCb.type = 'checkbox'; cuCb.checked = e.countUp !== false; cuCb.style.width = 'auto';
        cuCb.addEventListener('change', () => { e.countUp = cuCb.checked; commit(); });
        cuLbl.appendChild(cuCb); cuLbl.appendChild(document.createTextNode('Count up to the value (numbers only)'));
        body2.appendChild(cuLbl);
      }

      renderMapEventTiming(body2, e, obj, commit);
    });

    const addRow = document.createElement('div'); addRow.className = 'add-row'; addRow.style.marginTop = '4px'; wrap.appendChild(addRow);
    [['highlight', '+ Highlight'], ['zoom', '+ Zoom'], ['route', '+ Route'], ['stat', '+ Statistic']].forEach(([type, label]) => {
...
function renderMapPropsPanel(body, obj) {
  ...
  [{ v: 'world', label: 'World' }, { v: 'japan', label: 'Japan prefectures' }].forEach(s => {
    const b = document.createElement('button'); b.type = 'button'; b.style.flex = '1';
    b.className = 'small' + (cfg.scope === s.v ? ' primary' : ' ghost');
    b.textContent = s.label;
    b.addEventListener('click', () => { if (cfg.scope !== s.v) rebuildMap(obj, { scope: s.v, events: [] }); });
    scRow.appendChild(b);
  });
