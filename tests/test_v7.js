...
async function main() {
  ...
  console.log('1. chips include All + 5 categories:', chipLabels.length === 6 ? 'OK' : 'FAIL');

  const allTplBtns = await page.$$('#templateGrid .tpl-btn');
  console.log('2. "All" shows all 11 templates:', allTplBtns.length, allTplBtns.length === 11 ? 'OK' : 'FAIL');
  ...
  console.log('5. back to All restores full list:', restoredBtns.length, restoredBtns.length === 11 ? 'OK' : 'FAIL');
